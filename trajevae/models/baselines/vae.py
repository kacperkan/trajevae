...
class Model(BaseModel):
    def __init__(self, config: EasyDict, is_test_run: bool) -> None:
        ...
        self.mu = nn.Linear(self.e_mlp.out_dim, self.nh_rnn)
        self.logvar = nn.Linear(self.e_mlp.out_dim, self.nh_rnn)

        self.d_rnn = RNNUnit(
            self.num_joints * 3 * 2 + self.nh_rnn,
            self.nh_rnn,
            cell_type=self.cell_type,
        )
    ...
    def reparametrize(
        self, mu: torch.Tensor, logvar: torch.Tensor, std: float
    ) -> torch.Tensor:
