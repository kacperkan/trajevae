...
import numpy as np
import tqdm
from trajevae.utils.camera import (
    image_coordinates,
    project_to_2d,
    world_to_camera,
)
from trajevae.utils.general import wrap
from trajevae.utils.h36m import Human36mDataset
...
def process():
    ...
    # Convert dataset preprocessed by Martinez et al.
    # in https://github.com/una-dinosauria/3d-pose-baseline
    parser.add_argument(
        "--from-archive",
        default="",
        type=str,
    ...
    # Convert dataset from original source, using files converted to
    # .mat (the Human3.6M dataset path must be specified manually)
    ...
    if os.path.exists(os.path.join(args.output_dir, output_filename + ".npz")):
        print(
            "The dataset already exists at",
            os.path.join(args.output_dir, output_filename + ".npz"),
        )
        exit(0)
    ...
    if args.from_archive:
        ...
        print("Saving...")
        np.savez_compressed(
            os.path.join(args.output_dir, output_filename), positions_3d=output
        )
        ...
        for subject in tqdm.tqdm(subjects, desc="Subject"):
            output[subject] = {}
            file_list = glob(
                args.from_source_cdf
                + "/"
            ...
            for f in tqdm.tqdm(file_list, desc="File", leave=False):
                action = os.path.splitext(os.path.basename(f))[0]
    ...
    for subject in dataset.subjects():
        ...
        for action in dataset[subject].keys():
            ...
            for cam in anim["cameras"]:
                ...
                pos_2d = wrap(
                    project_to_2d, pos_3d, cam["intrinsic"], unsqueeze=True
                )
